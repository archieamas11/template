...
// 🏎️ Move regex to top-level scope for performance
	...
	const initialColumnFilters: ColumnFiltersState = React.useMemo(() => {
		...
		return Object.entries(filterValues).reduce<ColumnFiltersState>((filters, [key, value]) => {
			if (value !== null) {
				...
				} else if (typeof value === 'string' && nonAlphaNumericRegex.test(value)) {
					// 🧩 Split string by non-alphanumeric chars
					processedValue = value.split(nonAlphaNumericRegex).filter(Boolean);
				} else {
					processedValue = [value];
				}
